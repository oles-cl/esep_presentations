...
# ¿Qué es el Estudio de Percepciones de Seguridad y Policías en Chile (ESEP)?
...
Los resultados presentados a continuación fueron ponderados por probabilidades inversas de selección más a calibración por raking usando las variables de región, área, sexo, tramo etario y nivel educacional según las proyecciones poblacionales del Instituo Nacional de Estadísticas.
...
data_ola0 |>
    mutate(b1=to_label(b1), wt=ponderador)|>
  count(b1) |>
...
data_ola0 |>
  ...
  count(Variable=c1_1, wt=ponderador)|>
...
data_ola0 |>
  count(Variable=to_label(c1_2), wt=ponderador)|>
  mutate(prop=(n/nrow(data_ola0)*100)|>round(1),
...
data_ola0 |>
  count(Variable=to_label(c3), wt=ponderador)|>
  mutate(prop=(n/nrow(data_ola0)*100)|>round(1),
...
```{r}
#| label: fig-c2_1-demo
#| fig-cap: "(Cruce demográfico) Los carabineros toman decisiones justas e imparciales en los casos de los que se ocupan"
#| echo: false
#| warning: false
...
```{r}
#| label: fig-c2_2-demo
#| fig-cap: "(Cruce demográfico) Los carabineros tratan a las personas con respeto"
#| echo: false
#| warning: false
...
data_ola0 |>
  count(Variable=to_label(c4_1), wt=ponderador)|>
  mutate(prop=(n/nrow(data_ola0)*100)|>round(1))|>
...
data_ola0 |>
  count(Variable=to_label(c4_2), wt=ponderador) |>
  mutate(prop=(n/nrow(data_ola0)*100)|>round(1),
         Item="Toman decisiones justas e imparciales en los casos de los que se ocupan?") |>
...
```{r}
#| label: fig-c4_1-demo
#| fig-cap: "(Cruce demográfico) Los funcionarios de la PDI toman decisiones justas e imparciales en los casos de los que se ocupan"
#| echo: false
#| warning: false
...
```{r}
#| label: fig-c4_2-demo
#| fig-cap: "(Cruce demográfico) Los funcionarios de la PDI tratan a las personas con respeto en los casos de los que se ocupan"
#| echo: false
#| warning: false
...
rbind(
data_ola0 |>
  count(Variable=to_label(c5_1), wt=ponderador) |>
  mutate(prop=(n/nrow(data_ola0)*100),
         Item="Carabineros de Chile",
         prop=round(prop,1)),
data_ola0 |>
  count(Variable=to_label(c5_2), wt=ponderador) |>
  mutate(prop=(n/nrow(data_ola0)*100),
...
#| fig-cap: "¿Podría Ud. decirme cuánta confianza tiene en la Policía de Investigaciones (PDI)? (Cruce demográfico)"
